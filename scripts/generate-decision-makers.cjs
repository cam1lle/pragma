...
const formatEntry = (d) => {
  const domainsStr = JSON.stringify(d.domains);
  return `  { name: '${d.name}', role: '${d.role}', org: '${d.org}', email: '${d.email}', domains: ${domainsStr}, orgType: '${d.orgType}', seniority: '${d.seniority}' },`
}

const output = `// Seed the Decision-Maker database with real-world contacts (${dm.length} entries)
...
fs.writeFileSync('src/prisma/seed-decision-makers.ts', output);
console.log('Generated seed file with', dm.length, 'decision makers');
